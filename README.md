...
인증(Authentication)과 인가(Authorization or 권한 부여) 기능을 지원하는 보안 프레임워크로써,  
...
지금까지는 회원가입을 하면, memberRepository의 save를 통해 db에 저장되었음.(세션기반 인증 = 서버, db에 유저정보 담는 방식)  
그러한 부담(요청시마다 db 봐야하는)을 클라이언트에게 넘기기 위해 제이슨 웹 토큰 JWT 기반의 인증을 배울 예정.  
토큰은 정보를 암호화한 상태로 담아서 클라이언트에 담아도 된다.  
JWT는 헤더, 페이로드, 시그니처 (앞 중간 뒤) 순서.  
헤더는 종류, 암호화 알고리즘  
페이로드는 정보, 권한, 기타정보  
시그니처는 salt 값으로 또다시 암호화  
  
여긴 auth 패키지의 jwt 패키지에서 테스트, 메인 둘다 구현해봄.  
...
액세스 토큰(Access Token) : 접근권한  
리프레시 토큰(Refresh Token) : 엑세스 토큰 새로고침  
...
Payload는 base64로 인코딩되기 때문에 토큰을 탈취하여 Payload를 디코딩하면 토큰 생성 시 저장한 데이터를 손쉽게 확인할 수 있으므로 Payload에는 민감한 정보를 포함하지 않아야 한다.  
